...
function Calendar({
    ...
    return (
        <DayPicker
            ...
            /* ─── NUEVO: se sobrescribe Chevron en vez de IconLeft|IconRight ─── */
            components={{
                Chevron: ({ orientation, className, ...rest }) => {
                    const shared = cn("h-4 w-4", className)

                    if (orientation === "left") {
                        return <ChevronLeft className={shared} {...rest} />
                    }
                    if (orientation === "right") {
                        return <ChevronRight className={shared} {...rest} />
                    }
                    // fallback obligatorio para satisfacer el tipo Element
                    return <span className={shared} {...rest} />
                },
            }}
