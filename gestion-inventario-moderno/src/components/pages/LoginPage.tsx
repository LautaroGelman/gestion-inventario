...
/* --------------------------------------------------------------------------
 * Tipado del AuthResponse del backend (para pasar objeto completo al AuthContext)
 * -------------------------------------------------------------------------- */
type AuthResponse = {
    token: string;
    clientId: number | string;
    employeeId: number | string;
    sucursalId: number | string | null; // puede venir null si ROLE_PROPIETARIO
    roles: string[] | string; // el AuthContext normaliza ROLE_
    clientName?: string;
    employeeName?: string;
};
...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // 🔸 Pasamos el OBJETO COMPLETO al AuthContext (soporta roles y sucursalId)
            login({
                token: data.token,
            ...
            // El redirect lo maneja el useEffect al detectar user !== null
        } catch (err: any) {
            const msg =
