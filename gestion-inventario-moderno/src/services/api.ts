...
const baseURL: string =
    process.env.NODE_ENV === 'development'
        ? '/api'                                 // usa el proxy de Next.js en desarrollo
        : process.env.NEXT_PUBLIC_API_URL ?? ''; // apunta a tu backend en producción

// Instancia Axios para JSON
const apiClient: AxiosInstance = axios.create({
    baseURL,
    headers: { 'Content-Type': 'application/json' },
});

// Interceptor para adjuntar JWT desde localStorage en cada petición
...
// Empleados
export const getEmployees = (clientId: string) =>
    apiClient.get(`/client-panel/${clientId}/employees`);
...
export const createEmployee = (
    ...
    apiClient.post(
        `/client-panel/${clientId}/employees`,
        data
    );
...
export const updateEmployee = (
    ...
    apiClient.put(
        `/client-panel/${clientId}/employees/${employeeId}`,
        data
    );
